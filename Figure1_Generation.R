...
bioc_pkgs <- c(
  'minfi', 'IlluminaHumanMethylationEPICanno.ilm10b5.hg38',
  'ConsensusClusterPlus', 'sva', 'ComplexHeatmap', 'rtracklayer', 'annotatr'
)
cran_pkgs <- c(
  'dplyr', 'magrittr', 'readr', 'xlsx', 'reshape2', 'ggplot2', 'cowplot'
)
...
dir.create('./Figures')
png('./Figures/QC_plot_AllSamples.png', height = 600, width = 600)
plotQC(qc) # QC plotting
dev.off()

# FLAGGED SAMPLE : ncol(142) :: Visualize beta-distributions 
ggbg <- function() {
...
### Reading in of clinical data
pd <- read.xlsx('./Supplementary_Table_S1.xlsx')
head(pd, 3)
pd <- pd[!is.na(pd$Sentrix_ID),] # Excluding any poor quality samples that were flagged
pd$IDAT <- paste0(pd$Sentrix_ID, '_', pd$Sentrix_Position)
table(pd$IDAT %in% colnames(beta)) # Should be all samples
beta <- beta[, pd$IDAT]
...
group1 <- c(pd$IDAT_ID[pd$Kcluster_5 == '1']) 
group2 <- c(pd$IDAT_ID[pd$Kcluster_5 != '1'])
...
group1 <- c(pd$IDAT_ID[pd$Kcluster_5 == '2']) 
group2 <- c(pd$IDAT_ID[pd$Kcluster_5 != '2'])
...
group1 <- c(pd$IDAT_ID[pd$Kcluster_5 == '3']) 
group2 <- c(pd$IDAT_ID[pd$Kcluster_5 != '3'])
...
group1 <- c(pd$IDAT_ID[pd$Kcluster_5 == '4']) 
group2 <- c(pd$IDAT_ID[pd$Kcluster_5 != '4'])
...
group1 <- c(pd$IDAT_ID[pd$Kcluster_5 == '5']) 
group2 <- c(pd$IDAT_ID[pd$Kcluster_5 != '5'])
...
## Reading in supplemental table 1 from publication
pd_expanded <- read_excel('./Materials/Supplementary_Table_S1.xlsx')
rownames(pd_expanded) <- pd_expanded$IDAT_ID
