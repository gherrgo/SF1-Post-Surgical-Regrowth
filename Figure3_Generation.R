...
dmp_sets <- read_excel("./Materials/Supplementary_File_S3.xlsx")
colnames(dmp_sets)[1] <- 'Probe_ID'

############################################################
# 2. Publicly available data preprocessing & application of
# classifier. Processing standards chosen according to
# publication; end goal is a beta_matrix (e.g., beta_silva).
############################################################
...
############################################################
beta_silva <- beta_silva[dmp_sets$Probe_ID, ] # Subsetting the public data across the identified CpG sites
