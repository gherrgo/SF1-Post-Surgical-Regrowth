...
pd <- pd[, c('Sample_ID', 'Kcluster_5')]
followUp <- read_excel('./Materials/Supplementary_File_S1.xlsx',
                       sheet = 'Main Cohort Follow Up Data')
pd <- merge(followUp, pd, by = 1) # 737 time points for 111 non-excluded samples
pd$event <- ifelse(pd$`Postoperative Tumor Volume` < pd$`Postoperative Volume`, 1, 0)

###############################################################################################
# 2. Collapse the dataset by 'Sample_ID' to capture the first event time or the last time point
###############################################################################################
pd_collapsed <- pd %>%
  group_by(Sample_ID) %>%
